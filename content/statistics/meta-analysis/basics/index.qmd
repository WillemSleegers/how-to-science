...
Meta-analysis combines estimates from a related set of studies into a single estimate, weighting each study by how precisely it estimated the effect. The sections below cover the basics of conducting a meta-analysis.
...
Doing so requires accounting for sampling error: each study estimates the effect from a limited number of participants, and individual participants vary in their responses for reasons that have nothing to do with the effect itself. A different set of participants, even under an otherwise identical study, would produce a somewhat different estimate. The standard approach to meta-analysis models this directly, treating each study's estimate as normally distributed around the effect, with a spread equal to that study's sampling variance.
