...
Rules of thumb are one way to determine an appropriate sample size, but the general consensus seems to be that rules of thumb are a bad idea [@maccallum1999]. The reason for that is that the rules of thumb only involve one of two factors: a set number of participants or a particular participant-to-item ratio. The reality is that the appropriate sample size is dependent on many more factors, such as the:
